...
export default function usePivoData(data: SemanticData, mappings: { [keyInResult: string]: DataSemantics }): { [key: string]: any } {
  const [resolvedData, setResolvedData] = useState<{ [key: string]: any }>({})

  useEffect(() => {
    const keysInResult = Object.keys(mappings)
    const promises = Object.values(mappings).map(semanticKey => data.getOneValue(semanticKey))
